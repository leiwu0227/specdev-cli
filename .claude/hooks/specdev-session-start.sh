    ...
    try {
      ...
      if (state.state === "idle" || workflow === "none") {
        process.stdout.write(
          "SpecDev installed. Classify the user request before creating state: Direct for questions, read-only inspection, and small non-behavioral documentation writes; or explicitly user-selected Roadmap, Adhoc, Discussion, Assignment, or Mission. Roadmap is stateless and may write only forecast.md, todo.md, and Markdown under designs/ after exact user approval. Designs stay below 800 words; Forecast contains dependency-ordered design-derived gaps below 200 words with provenance, while Todo contains dependency-ordered, user-prioritized non-architecture work below 200 words without provenance. Neither list grants implementation authority. Selecting another lane immediately supersedes Roadmap without an exit command. Direct writes create no workflow state, receipt, or automatic commit. Never silently create an Assignment for every request." +
