...
import { readCurrent as readRippleCurrent, writeCheckpoint, writeCurrent as writeRippleCurrent } from 'ripplegraph'
...
import { assertReviewWaiverEvidence, validateDeliveryArtifacts } from '../src/utils/delivery-artifacts.js'
import { assignmentContractTemplate, normalizeReviewPolicy, reviewPolicyFromFlags } from '../src/utils/assignment-vnext.js'
import { classifyWorkspaceChanges, parseGitPorcelainPaths, workspaceChangeSummaryLines } from '../src/utils/workspace-changes.js'
...
try {
  ...
  writeFileSync(join(specdevPath, 'agents.yaml'), `worker:\n  provider: codex\n  model: repo-worker\n  effort: medium\n  network: true\n  timeout: 30m\nreviewer:\n  provider: claude\n  model: repo-reviewer\n  effort: high\n  timeout: 10m\n`)
  writeFileSync(join(specdevPath, 'cache', 'agents.local.yaml'), `reviewer:\n  model: local-reviewer\n`)
  ...
  assert.throws(() => validateProfile('worker', {
    provider: 'codex', model: 'gpt-test', effort: 'medium', network: 'yes', timeout: '1m',
  }), /network must be true or false/)
  assert.throws(() => validateProfile('worker', {
    provider: 'claude', model: 'sonnet', effort: 'medium', network: true, timeout: '1m',
  }), /supported only for the codex worker/)
  assert.throws(() => validateProfile('reviewer', {
    provider: 'codex', model: 'gpt-test', effort: 'medium', network: true, timeout: '1m',
  }), /supported only for the codex worker/)

  const codexReview = buildProviderInvocation({
    profile: { provider: 'codex', model: 'gpt-test', effort: 'high' },
    role: 'reviewer', cwd: root, resultPath: join(specdevPath, 'cache', 'result.md'),
  })
  assert.equal(codexReview.command, 'codex')
  assert.equal(codexReview.args.includes('read-only'), true)
  assert.equal(codexReview.args.some((arg) => arg.includes('dangerously')), false)
  ...
  const activity = summarizeAttemptActivity([
    ...
    {
      kind: 'worker', status: 'blocked', provider: 'codex', duration_ms: 100,
      usage: { provider_reported_tokens: 120 },
    },
    {
      kind: 'reviewer', status: 'completed', provider: 'claude', duration_ms: 200,
  ...
  assert.deepEqual(activity, {
    ...
    provider_attempts: {
      total: 2, completed: 1, failed: 0, blocked: 1, interrupted: 0, running: 0,
    },
    elapsed_ms: 1_000,
  ...
  const ids = await Promise.all(Array.from({ length: 8 }, () => reserveEntityId(specdevPath, 'assignment')))
  assert.equal(new Set(ids).size, ids.length)
  assert.deepEqual([...ids].sort(), ['00001', '00002', '00003', '00004', '00005', '00006', '00007', '00008'])
  ...
  assert.deepEqual(reviewPolicyFromFlags({ 'implementation-review': 'waived' }), { brainstorm: 'optional', implementation: 'waived' })
  assert.throws(() => reviewPolicyFromFlags({ 'brainstorm-review': 'never' }), /Invalid brainstorm review policy/)
  assert.match(assignmentContractTemplate({ description: 'Small fix' }), /fewest independent observable acceptance criteria \(normally 1-5\)/)
  assert.deepEqual(
  ...
  const attempt = await createAttemptRecord(specdevPath, {
    kind: 'reviewer', workspace: '.', discussion: 'D00001', provider: 'claude', model: 'opus',
  ...
  const completedAttempt = await updateAttemptRecord(specdevPath, attempt.id, { status: 'completed' })
  const repeatedCompletion = await updateAttemptRecord(specdevPath, attempt.id, { status: 'completed' })
  assert.equal(repeatedCompletion.ended_at, completedAttempt.ended_at)

  assert.deepEqual(normalizeFtsTerms('parser, parser OR foo_bar src/api.ts ???'), ['parser', 'OR', 'foo_bar', 'src/api.ts'])
  ...
  const supersededFaq = await searchKnowledgeIndex(specdevPath, 'retiredrouter', { scope: 'history' })
  ...
  const unreferencedAssignmentPath = join(distillationSpecdevPath, 'assignments', '00009_distill-me')
  mkdirSync(unreferencedAssignmentPath, { recursive: true })
  writeFileSync(join(unreferencedAssignmentPath, 'status.json'), JSON.stringify({ status: 'completed', mission: 'M00009' }))
  writeFileSync(join(unreferencedAssignmentPath, 'outcome.md'), '# Distill me\n\nReusable repository behavior.\n')
  let distillation = await buildKnowledgeDistillationBrief(distillationSpecdevPath, { now: '2026-07-21' })
  assert.equal(distillation.unreferenced_sources.some((source) => source.assignment === '00009_distill-me'), true)
  assert.equal(distillation.stale_faqs.some((faq) => faq.path === 'knowledge/faq/stale.md'), true)
  const missionDistillation = await buildKnowledgeDistillationBrief(distillationSpecdevPath, { mission: 'M00009', now: '2026-07-21' })
  ...
  distillation = await buildKnowledgeDistillationBrief(distillationSpecdevPath, { now: '2026-07-21' })
  assert.equal(distillation.unreferenced_sources.some((source) => source.assignment === '00009_distill-me'), false)
  ...
  const missionAttempt = await createAttemptRecord(specdevPath, {
    kind: 'mission-controller', workspace: '.', mission: 'M00001',
  })
  ...
  assert.deepEqual(catalog.map((entry) => entry.id), ['frontend', 'api-security'])
  assert.equal((await resolveGuides(specdevPath, ['api-security'], { phase: 'brainstorm' })).length, 1)
  await assert.rejects(resolveGuides(specdevPath, ['frontend'], { phase: 'brainstorm' }), /does not apply/)
  ...
  writeFileSync(join(deliveryPath, 'design', 'plan.md'), '# Plan\n\nImplementation Guides: [api-security]\nReview Guides: [frontend]\n\n## Tasks\n\n- T-1 covers AC-1.\n')
  writeFileSync(join(deliveryPath, 'implementation', 'progress.json'), JSON.stringify({
    version: 1,
  ...
  assert.deepEqual(delivery.progress.selected_guide_versions.review, [{ id: 'frontend', version: '1' }])
  ...
  await assert.rejects(validateDeliveryArtifacts(specdevPath, deliveryPath, ['AC-1']), /unknown acceptance criterion AC-99/)
  writeFileSync(join(deliveryPath, 'design', 'plan.md'), '# Plan\n\n**Implementation Guides:** [api-security]\n**Review Guides:** [frontend]\n\n## Tasks\n\n- T-1 covers AC-1.\n')
  writeFileSync(join(deliveryPath, 'outcome.md'), '# Outcome\n\nNo acceptance table.\n')
  await assert.rejects(validateDeliveryArtifacts(specdevPath, deliveryPath, ['AC-1']), /no final Passed/)

  const originalQueue = {
    version: 1,
    assignments: [
      { id: '00009', title: 'Completed child', kind: 'change', status: 'completed', outcome: '.specdev/assignments/00009_done/outcome.md' },
  ...
  await assert.rejects(
    validateAndReserveReplannedQueue(specdevPath, originalQueue, {
      ...originalQueue,
      assignments: [{ ...originalQueue.assignments[0], title: 'Illegally changed' }, originalQueue.assignments[1]],
    }),
