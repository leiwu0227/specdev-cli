...
console.log('\nknowledge search missing index:')
writeFixture()
result = runCmd(['knowledge', 'search', 'bounded memory', `--target=${TEST_DIR}`])
assert(result.status === 1, 'knowledge search without index fails')
assert(result.stderr.includes('Run: specdev knowledge index'), 'missing index error gives fix')
