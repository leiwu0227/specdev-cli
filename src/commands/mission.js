...
async function createMission(args, flags) {
  ...
  let guided
  try {
    guided = startGuidedRun(targetDir, 'mission-lifecycle', { strict: true, expectedNode: 'create-mission' })
  } catch (error) {
    return fail(flags, error.message)
  }
  ...
  let contract = assignmentContractTemplate({ description: objective, kind: 'mission', sourceDiscussion: source })
  ...
  const stepped = stepGuidedNode(targetDir, 'create-mission', { id, path: relativeToRepo(targetDir, missionPath), objective })
  if (!stepped.synchronized) return fail(flags, `Could not record Mission ${id} creation; resume it with specdev mission run ${id}`)
  return emit(flags, {
    command: 'mission create', version: 1, status: 'brainstorming', id, name,
    path: relativeToRepo(targetDir, missionPath), branch,
    next_action: `Finish brainstorm/contract.md, including the exact final verification command, then run specdev mission run ${id}.`,
  })
}
...
async function runMission(selector, flags) {
  ...
  if (mission.status === 'completed') {
    return emit(flags, {
      command: 'mission run', version: 1, status: 'completed', mission: mission.id,
      branch: mission.branch, base_branch: mission.base_branch,
  ...
  try {
    ...
    if (graph.position?.node === 'create-mission') {
      ...
      if (!recovered.synchronized) throw new Error(`Could not recover Mission ${mission.id} creation`)
      graph = getState({ workflowRoot: workflowRootFor(targetDir) })
    }
    const childReviewOverrideRequested = Boolean(flags['override-review']) &&
    ...
    const childReviewOverrideId = childReviewOverrideRequested
      ? (await readMissionQueue(missionPath))?.assignments?.find((item) => item.status === 'running')?.id || null
      : null
    ...
    if (graph.position.node === 'approve-mission') {
      ...
      if (!contract.valid) return fail(flags, contract.errors.join('; '))
      const git = await gitSnapshot(targetDir)
      if (!git.branch) return fail(flags, 'Mission approval requires a named Git base branch; switch from detached HEAD first')
      ...
      if (checkpointed?.contract_hash !== contract.hash) {
        ...
        if (!reset.synchronized) throw new Error('Could not return the Mission contract to Brainstorm for re-checkpointing')
        const refreshed = stepGuidedNode(targetDir, 'brainstorm', {
          contract: relativeToRepo(targetDir, contract.path),
          contract_hash: contract.hash,
        })
        if (!refreshed.synchronized) throw new Error('Could not checkpoint the changed Mission contract')
        graph = getState({ workflowRoot: workflowRootFor(targetDir) })
        recheckpointed = true
      }
      ...
      if (!flags.approve || recheckpointed) {
        ...
        return emit(flags, {
          command: 'mission run', version: 1, status: 'awaiting_approval', mission: mission.id,
          contract_hash: contract.hash, branch: mission.branch, dirty_paths: git.dirty_paths,
          review: review ? {
      ...
      if (review && review.result.frontmatter.verdict !== 'approved' && !flags['override-review']) {
        return fail(flags, 'The current Mission Brainstorm review is not approved. Address it and rerun review, or explicitly use --override-review.')
      }
      ...
      if (!decision.synchronized) throw new Error('Could not record Mission approval in the focused workflow')
      ...
      delete mission.blocker
      await writeMission(missionPath, mission)
      await retireTransientArtifact(targetDir, specdevPath, join(missionPath, 'review', 'brainstorm-baseline.md'))
    }

    if (graph.position.node !== 'approve-mission') {
      const contract = await validateMissionContract(missionPath)
      if (!contract.valid || contract.hash !== mission.approved_contract_hash) {
        throw new Error('Mission contract changed after approval; restore the approved contract or create a new Mission for changed authority')
      }
      await establishMissionBranch(targetDir, mission)
    ...
    if (
      ...
      await withSuppressedOutput(() => checkpointMission(mission.id, { target: targetDir, json: true }))
    ...
    try {
      const payload = await driveMission({ ...context, mission, flags, childReviewOverrideId, controller })
      ...
      if (mission.status === 'completed') {
        return fail(flags, `Mission delivery completed but its completion checkpoint failed: ${error.message}. Retry with specdev mission checkpoint ${mission.id}`)
      }
      await updateAttemptRecord(specdevPath, controller.id, { status: 'blocked', error: error.message })
  ...
  while (true) {
    ...
    if (node === 'mission-review') {
      ...
      if (!approved) {
        ...
        if (reviewState.round >= 2) throw new Error('Mission review failed its verification rerun; user direction is required')
        mission.pending_replan = {
        ...
        stepGuidedNode(targetDir, 'mission-review', {
          approved: false,
          verdict: '.specdev/' + relativeToRepo(specdevPath, join(missionPath, 'review', 'mission-verdict.md')),
    ...
    if (node === 'final-verification') {
      ...
      if (!result.passed) {
        ...
        await writeMission(missionPath, mission)
        if (mission.verification_failures > 1) throw new Error('Final verification failed again; user direction is required')
        continue
      }
      ...
      const checkpoint = await withSuppressedOutput(() => checkpointMission(mission.id, {
        target: targetDir,
        json: true,
      }))
    ...
    if (graph.status === 'completed' || graph.run?.status === 'completed') {
      return emit(flags, { command: 'mission run', version: 1, status: 'completed', mission: mission.id })
    }
...
async function designMission(context) {
  ...
  const result = recovered || await runSpawnedAgent({
    targetDir, specdevPath, role: 'worker', profile, mission: mission.id,
    resultPath,
    resultKind: 'worker',
    prompt: [
...
async function runMissionChild(context) {
  ...
  if (graph.position.node === 'create-assignment') {
    ...
    if (existing) {
      ...
      if (status?.id !== child.id || status?.mission !== mission.id) {
        throw new Error(`Existing child Assignment ${child.id} does not belong to Mission ${mission.id}`)
      }
      ...
      if (!recovered.synchronized) throw new Error(`Could not recover child Assignment ${child.id} creation`)
    } else {
      await withSuppressedOutput(() => assignmentCommand([child.title], {
        target: targetDir, mission: mission.id, id: child.id, kind: child.kind || 'change', slug: slugify(child.title), json: true,
        'brainstorm-review': queue.design_mode === 'single' && queue.assignments.length === 1 ? 'optional' : 'required',
  ...
  if (graph.position.node === 'brainstorm') {
    ...
    if (!contract.valid) throw new Error(`Child ${child.id} contract invalid: ${contract.errors.join('; ')}`)
    stepGuidedNode(targetDir, 'brainstorm', {
      contract: relativeToRepo(targetDir, contract.path), contract_hash: contract.hash,
    })
  }
  graph = getState({ workflowRoot: workflowRootFor(targetDir) })
  if (graph.position.node === 'approve-contract') {
    if (!singleFullScope) {
      const savedReview = await fse.readJson(join(assignmentPath, 'review', 'brainstorm-state.json')).catch(() => null)
      let review
      if (savedReview?.status === 'blocked') {
        review = { status: 'blocked', material_divergence: savedReview.material_divergence ?? false }
      } else if (savedReview?.status === 'needs_changes') {
        await reviseChildContract(context, child, assignmentPath)
        review = await withSuppressedOutput(() => reviewBrainstorm({ target: targetDir, assignment: child.id, json: true }))
      } else {
        review = await withSuppressedOutput(() => reviewBrainstorm({ target: targetDir, assignment: child.id, json: true }))
      }
      ...
      if (!review) throw new Error(`Child ${child.id} Brainstorm review did not produce a durable verdict`)
      if (review?.status !== 'approved' && !childReviewOverride) {
        throw new Error(`Child ${child.id} Brainstorm review blocked; after explicit user direction, rerun with --override-review`)
      }
      if (review?.material_divergence === true && !childReviewOverride) {
        throw new Error(`Child ${child.id} Brainstorm materially diverges from its reviewed baseline; after explicit user direction, rerun with --override-review`)
      }
    }
    await assertApprovedMissionContract(missionPath, mission)
    if (!singleFullScope) {
      await withSuppressedOutput(() => checkpointCommand(['brainstorm'], {
        target: targetDir,
        assignment: child.id,
        json: true,
      }))
    ...
    const reviewPolicy = { brainstorm: singleFullScope ? 'optional' : 'required', implementation: 'required' }
    const approval = decideGuidedNode(targetDir, 'approve-contract', {
      approved: true, contract_hash: contract.hash, actor: `mission:${mission.id}`,
      approved_at: approvedAt, revision: (await gitSnapshot(targetDir)).revision || 'unborn',
      review_override: childReviewOverride,
      review_policy: reviewPolicy,
    })
    if (!approval.synchronized) throw new Error(`Could not record Mission approval for child ${child.id}`)
    await writeAssignmentStatus(assignmentPath, {
      review_policy: reviewPolicy,
      review_policy_frozen_at: approvedAt,
    })
    await retireTransientArtifact(targetDir, specdevPath, join(assignmentPath, 'review', 'brainstorm-baseline.md'))
    await retireTransientArtifact(targetDir, specdevPath, join(assignmentPath, 'review', 'brainstorm-author-result.md'))
    await retireTransientArtifact(targetDir, specdevPath, join(assignmentPath, 'review', 'brainstorm-revision-result.md'))
  }
  let result
  try {
    result = await withSuppressedOutput(() => implementCommand([], { target: targetDir, assignment: child.id, json: true }))
  } finally {
    await writeCurrentFocus(specdevPath, { kind: 'mission', id: mission.id })
  }
...
async function authorChildContract(context, child, assignmentPath) {
  ...
  const result = await runSpawnedAgent({
    targetDir, specdevPath, role: 'worker', profile, mission: mission.id, assignment: child.id,
    resultPath: join(assignmentPath, 'review', 'brainstorm-author-result.md'), resultKind: 'worker',
    prompt: [
      ...
      await optionalArtifactLine(targetDir, join(missionPath, 'review', 'mission-verdict.md'), 'Latest Mission review findings'),
      await optionalArtifactLine(targetDir, join(missionPath, 'review', 'final-verification.json'), 'Latest integrated verification receipt'),
...
async function completedOutcomeLines(missionPath, currentChildId) {
  ...
  return paths.length > 0
    ? ['Completed prerequisite outcomes:', ...paths]
    : []
}
...
async function childFollowUp(specdevPath, child) {
  ...
  const outcome = await fse.readFile(join(specdevPath, 'assignments', child.folder, 'outcome.md'), 'utf-8').catch(() => '')
  if (/^\|[^\n]*\|\s*(Failed|Blocked)\s*\|\s*$/mi.test(outcome)) return 'required'
  return 'none'
}
...
async function reviseChildContract(context, child, assignmentPath) {
  ...
  const result = await runSpawnedAgent({
    targetDir, specdevPath, role: 'worker', profile, mission: mission.id, assignment: child.id,
    resultPath: join(assignmentPath, 'review', 'brainstorm-revision-result.md'), resultKind: 'worker',
    prompt: [
...
async function reviewMission(context) {
  ...
  const profile = state.profile
    ? await resolveAgentProfile(specdevPath, 'reviewer', { ...state.profile, timeout: state.profile.timeout_ms })
    : await resolveAgentProfile(specdevPath, 'reviewer')
  const result = await runSpawnedAgent({
    targetDir, specdevPath, role: 'reviewer', profile, mission: mission.id,
    resultPath: verdictPath, resultKind: 'reviewer',
    guides: [{ id: 'specdev-review', version: '1', path: join(specdevPath, 'guides', 'review.md') }],
    prompt: [
  ...
  const nextState = {
    version: 1, round: state.round + 1, status: approved ? 'approved' : verdict === 'blocked' ? 'blocked' : 'needs_changes',
    profile, attempt: result.attempt.id, updated_at: new Date().toISOString(),
  }
  await fse.writeJson(statePath, nextState, { spaces: 2 })
  if (verdict === 'blocked') throw new Error('Mission reviewer is blocked; user direction is required')
  if (approved) {
    stepGuidedNode(targetDir, 'mission-review', {
      approved: true, verdict: relativeToRepo(targetDir, verdictPath), attempt: result.attempt.id,
    })
  }
  return approved
}
...
async function reusableSingleChildReview(context) {
  const { targetDir, specdevPath, missionPath, mission } = context
  const queue = await readMissionQueue(missionPath)
  if (queue?.design_mode !== 'single' || queue.assignments?.length !== 1 || mission.pending_replan) return null
  ...
  const state = await fse.readJson(join(assignmentPath, 'review', 'implementation-state.json')).catch(() => null)
...
async function replanMission(context) {
  ...
  let revised
  try {
    revised = await validateAndReserveReplannedQueue(specdevPath, original, await readMissionQueue(missionPath))
  } catch (error) {
    await writeMissionQueue(missionPath, original)
    throw error
  }
...
async function runFinalVerification(context) {
  ...
  if (!command || command !== authorized) {
    throw new Error('Mission final verification must exactly match the command approved in the Mission contract')
  }
  ...
  const receipt = {
    command, revision, scope: queue.final_verification?.scope || 'integrated',
    status: exitCode === 0 ? 'passed' : 'failed', duration_ms: Date.now() - started,
...
async function completeMission(context) {
  ...
  await writeMission(missionPath, mission)
  await fse.writeJson(join(missionPath, 'status.json'), { version: 1, status: 'completed', completed_at: mission.completed_at }, { spaces: 2 })
}
...
async function validateAndReserveQueue(specdevPath, missionPath) {
  ...
  for (const item of queue.assignments) {
    if (!String(item.title || '').trim()) throw new Error('Every Mission queue item requires a title')
    if (!ASSIGNMENT_KINDS.includes(item.kind || 'change')) {
      throw new Error(`Invalid Mission Assignment kind: ${item.kind}. Valid kinds: ${ASSIGNMENT_KINDS.join(', ')}`)
    }
    if (item.status !== 'pending') throw new Error('Every new Mission queue item requires status: pending')
  }
  const command = String(queue.final_verification?.command || '').trim()
  ...
  if (!contract.valid || command !== authorizedVerificationCommand(contract.content)) {
    throw new Error('Mission Design final_verification.command must exactly match the approved Mission contract')
  }
  ...
  if (new Set(suppliedIds).size !== suppliedIds.length || suppliedIds.some((id) => !/^\d{5}$/.test(id))) {
...
async function recoverCompletedDesignResult({ targetDir, specdevPath, mission, queuePath, resultPath }) {
  ...
  const attempt = attempts.reverse().find((candidate) =>
    candidate.status === 'completed' && candidate.result_path === expectedPath
  )
...
async function validateMissionContract(missionPath) {
  ...
  if (result.valid) {
    ...
    if (!executionShape) {
      result.valid = false
      result.errors.push('Mission execution shape requires exactly one `- Initial child plan: single|planned` line')
...
function missionExecutionShape(content) {
  const values = [...String(content || '').matchAll(/^\s*-\s+Initial child plan:\s*(single|planned)\s*$/gmi)]
    .map((match) => match[1].toLowerCase())
  return values.length === 1 ? values[0] : ''
}

function missionSplitReason(content) {
  const reason = String(content || '').match(/^\s*-\s+Split reason:\s*(.+)\s*$/mi)?.[1]?.trim() || ''
  return reason && reason.toLowerCase() !== 'none' ? reason : ''
}

function contractSection(content, heading) {
  const escaped = heading.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  return String(content || '').match(new RegExp(`^##\\s+${escaped}\\s*$([\\s\\S]*?)(?=^##\\s+|(?![\\s\\S]))`, 'mi'))?.[1] || ''
}
...
function authorizedVerificationCommand(content) {
  const commands = [...String(content || '').matchAll(/^\s*-\s+Command:\s+`([^`]+)`\s*$/gmi)]
...
async function establishMissionBranch(targetDir, mission) {
  ...
  const localExists = await gitSucceeds(targetDir, ['show-ref', '--verify', '--quiet', `refs/heads/${mission.branch}`])
  const remoteBranch = `origin/${mission.branch}`
  const remoteExists = await gitSucceeds(targetDir, ['show-ref', '--verify', '--quiet', `refs/remotes/${remoteBranch}`])
  if (localExists) {
    if (git.dirty_paths.length > 0) throw new Error(`Wrong branch with a dirty worktree. Switch to ${mission.branch} manually.`)
    await execFile('git', ['switch', mission.branch], { cwd: targetDir })
  } else if (remoteExists) {
    if (git.dirty_paths.length > 0) throw new Error(`Wrong branch with a dirty worktree. Switch to ${mission.branch} manually.`)
    await execFile('git', ['switch', '--track', '-c', mission.branch, remoteBranch], { cwd: targetDir })
  } else {
...
async function focusMissionRun({ targetDir, mission }) {
  ...
  if (current.status === 'ok' && current.run?.status === 'active') {
    if (current.run.id !== mission.run_id) throw new Error('Another focused workflow is active; finish or suspend it first')
    return
  }
...
async function claimMissionController(context, flags) {
  ...
  const running = await listAttemptRecords(specdevPath, { kind: 'mission-controller', mission: mission.id, status: 'running' })
  for (const record of running) {
    const liveness = await attemptLiveness(specdevPath, record.id)
    if (liveness.state === 'live_local') return fail(flags, `Mission ${mission.id} already has a live local controller`)
    if (!flags.takeover) return fail(flags, `Mission ${mission.id} has an interrupted/ambiguous controller ${record.id}; rerun with --takeover`)
    await updateAttemptRecord(specdevPath, record.id, { status: 'interrupted', error: 'explicit takeover' })
    await clearLocalProcessMarker(specdevPath, record.id)
  }
  const attempt = await createAttemptRecord(specdevPath, {
    kind: 'mission-controller', mission: mission.id, workspace: '.',
...
async function pauseMission(selector, flags) {
  const context = await missionContext(selector, flags)
  if (!context) return null
  const running = await listAttemptRecords(context.specdevPath, { kind: 'mission-controller', mission: context.mission.id, status: 'running' })
  for (const record of running) {
    ...
    await updateAttemptRecord(context.specdevPath, record.id, { status: 'interrupted', error: 'mission paused' })
  ...
  return emit(flags, {
    command: 'mission pause', version: 1, status: 'paused', mission: context.mission.id,
...
async function missionStatus(selector, flags) {
  ...
  const graph = listRuns({ workflowRoot: workflowRootFor(context.targetDir) }).runs.find((run) => run.id === context.mission.run_id)
  const phase = graph?.position?.node || null
  const runningAttempts = await listAttemptRecords(context.specdevPath, {
    kind: 'mission-controller', mission: context.mission.id, status: 'running',
  })
  const controllerStates = await Promise.all(runningAttempts.map(async (record) => ({
    id: record.id,
  ...
  const effectiveStatus = interruptedController && context.mission.status === 'running'
    ? 'interrupted'
    : context.mission.status
  const blocker = context.mission.blocker || (interruptedController
    ? `Controller ${interruptedController.id} is ${interruptedController.liveness.state}; inspect it before takeover.`
    : null)
  const nextAction = missionNextAction(context.mission, phase, Boolean(liveController), interruptedController)
  ...
  const lastCheckpoint = context.mission.last_checkpoint
    ? {
        ...
        revision: await latestCheckpointRevision(context.targetDir, context.mission.branch, context.mission.id),
      }
    : null
  const activity = await missionActivitySummary(context.specdevPath, context.mission)
  return emit(flags, {
    command: 'mission status', version: 1, status: effectiveStatus,
    mission: context.mission.id, phase, branch: context.mission.branch,
    revision: await gitBranchRevision(context.targetDir, context.mission.branch) || currentGit.revision,
    ...
    counts: Object.fromEntries(['pending', 'running', 'completed', 'blocked', 'cancelled'].map((status) => [status, assignments.filter((item) => item.status === status).length])),
    ...
    outcome: await fse.pathExists(join(context.missionPath, 'outcome.md')) ? relativeToRepo(context.targetDir, join(context.missionPath, 'outcome.md')) : null,
  })
}
...
async function checkpointMission(selector, flags) {
  ...
  if (git.branch !== context.mission.branch) return fail(flags, `Mission checkpoint requires branch ${context.mission.branch}`)
  ...
  if (hasChanges) {
    ...
    await writeMission(context.missionPath, context.mission)
    await execFile('git', ['add', '--', relativeToRepo(context.targetDir, join(context.missionPath, 'mission.yaml'))], { cwd: context.targetDir })
    await execFile('git', ['commit', '-m', `specdev: checkpoint ${context.mission.id}`], { cwd: context.targetDir })
  }
  if (flags.push) {
    await execFile('git', ['push', '-u', 'origin', context.mission.branch], { cwd: context.targetDir })
  }
  const revision = (await gitSnapshot(context.targetDir)).revision
  return emit(flags, {
    command: 'mission checkpoint', version: 1, status: 'ok', mission: context.mission.id,
    committed: hasChanges, pushed: Boolean(flags.push), revision,
  })
}
...
async function unstageIncompleteDiscussions(context) {
  ...
  for (const entry of await fse.readdir(root, { withFileTypes: true })) {
    ...
    if (hasHead) {
      ...
      await execFile('git', ['rm', '--cached', '-r', '--ignore-unmatch', '--', path], { cwd: context.targetDir })
    }
  }
}
...
async function unstageIncompleteTestAudits(context) {
  ...
  for (const entry of await fse.readdir(root, { withFileTypes: true })) {
    ...
    if (hasHead) await execFile('git', ['restore', '--staged', '--', path], { cwd: context.targetDir })
    else await execFile('git', ['rm', '--cached', '-r', '--ignore-unmatch', '--', path], { cwd: context.targetDir })
  }
}
...
async function resolveSourceDiscussion(targetDir, specdevPath, selector, flags) {
  ...
  if (!resolved || resolved.error) { fail(flags, `Discussion not found: ${selector}`); return null }
  const id = resolved.name.match(/^D\d{4,5}/)?.[0]
  const call = readGuidedCall(targetDir, id)
  if (!call.synchronized || call.state.status !== 'completed') { fail(flags, `Discussion ${id} must be completed before promotion`); return null }
  ...
  if (!completedHash || completedHash !== currentHash) {
    fail(flags, `Discussion ${id} changed after completion; restore its completed artifacts or create a new Discussion`)
    return null
  }
  return {
    id,
...
async function objectiveFromDiscussion(source) {
  ...
  return String(content).match(/^#\s+(.+)$/m)?.[1]?.trim() || `Promote discussion ${source.id}`
}
...
async function gitSucceeds(cwd, args) {
  try { await execFile('git', args, { cwd }); return true } catch { return false }
}
...
async function latestCheckpointRevision(cwd, branch, missionId) {
  try {
    const { stdout } = await execFile('git', [
      'log', branch, '-1', '--format=%H', '--fixed-strings', `--grep=specdev: checkpoint ${missionId}`,
    ], { cwd })
    return stdout.trim() || null
  } catch {
    return null
...
function missionNextAction(mission, phase, liveController, interruptedController = null) {
  ...
  if (interruptedController) return `Inspect ${interruptedController.id}, then run specdev mission run ${mission.id} --takeover.`
  if (mission.status === 'blocked') return mission.next_action || `Resolve the blocker, then run specdev mission run ${mission.id}.`
  ...
  if (phase === 'brainstorm') return `Finish the Mission contract, then run specdev mission run ${mission.id}.`
...
async function missionActivitySummary(specdevPath, mission) {
  ...
  return attemptActivitySummary(specdevPath, { mission: mission.id }, {
...
function emit(flags, payload) {
  if (flags.json) console.log(JSON.stringify(payload, null, 2))
    ...
    if (payload.review?.stale) console.log(`Review note: the saved verdict covered older contract hash ${payload.review.reviewed_contract_hash}`)
    ...
    if (payload.last_checkpoint) console.log(`Last checkpoint: ${payload.last_checkpoint.revision || payload.last_checkpoint.created_at || payload.last_checkpoint.base_revision}`)
    if (payload.activity) {
      console.log(`Orchestration Attempts: ${payload.activity.orchestration_attempt_count}`)
      console.log(`Provider agent Attempts: ${formatProviderAttemptOutcomes(payload.activity.provider_attempts)}`)
      console.log(`Elapsed: ${formatDuration(payload.activity.elapsed_ms)}`)
      if (payload.activity.provider_reported_tokens !== null) console.log(`Provider-reported tokens: ${payload.activity.provider_reported_tokens}`)
    }
...
function fail(flags, message) {
  if (flags?.json) console.log(JSON.stringify({ command: 'mission', version: 1, status: 'error', error: message }))
