...
export async function continueCommand(flags = {}) {
  ...
  if (!hasWorkspaceEngine(targetDir)) return fail(flags, 'Workflow registry unavailable; run specdev update')
  ...
  const payload = focus?.kind === 'mission'
    ? { command: 'continue', version: 2, status: 'ok', kind: 'mission', id: focus.id, next_action: `specdev mission status ${focus.id}` }
    : focus?.kind === 'discussion'
      ? { command: 'continue', version: 2, status: 'ok', kind: 'discussion', id: focus.id, next_action: `specdev discussion ${focus.id}` }
      : { command: 'continue', version: 2, status: 'idle', kind: focus?.kind || null, id: focus?.id || null, next_action: 'specdev do "<intent>"' }
...
function fail(flags, message) {
  if (flags.json) console.log(JSON.stringify({ command: 'continue', version: 2, status: 'error', error: message }))
