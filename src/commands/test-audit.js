...
import {
  reserveTestAuditId,
  resolveTestAuditSelector,
  testAuditArtifactHash,
} from '../utils/test-audit.js'
...
export async function testAuditCommand(positionalArgs = [], flags = {}) {
  ...
  const selector =
    positionalArgs.length === 1 && /^TA\d{5}$/.test(positionalArgs[0]) ? positionalArgs[0] : null
  ...
  if (!scope)
    return fail(
      flags,
      'Usage: specdev test-audit "<test scope>" | specdev test-audit TA00001 [--complete] | specdev test-audit --list'
    )
  ...
  await fse.writeFile(
    ...
    assignmentContractTemplate({
      description: `Remove only redundant tests proven unnecessary by ${id}: ${scope}`,
      kind: 'refactor',
    }),
    'utf-8'
  )
  const input = {
    id,
    path: relativeToRepo(targetDir, auditPath),
    scope,
  ...
  return emit(flags, {
    ...
    start_revision: startRevision,
    authority:
      'Product code and tests are read-only. Write only audit.md and assignment-contract.md in this Test Audit.',
...
async function resumeAudit(targetDir, specdevPath, selector, flags) {
  ...
  if (flags.complete) {
    ...
    return emit(flags, {
      ...payload(targetDir, resolved, 'completed'),
      completed_revision: completedRevision,
      artifact_hash: artifactHash,
...
async function validateAuditArtifacts(auditPath) {
  ...
  if (!(await fse.pathExists(auditPathname))) errors.push('audit.md is missing')
    ...
    for (const heading of ['Candidates', 'Retained protection', 'Cost impact', 'Confidence']) {
      if (!new RegExp(`^##\\s+${heading}\\s*$`, 'mi').test(content))
        errors.push(`audit.md is missing ## ${heading}`)
    }
  ...
  if (!contract.valid)
    errors.push(...contract.errors.map((error) => `assignment-contract.md: ${error}`))
...
async function listAudits(targetDir, specdevPath, flags) {
  ...
  if (flags.json)
    return emit(flags, { command: 'test-audit list', version: 1, status: 'ok', audits })
...
function fail(flags, message) {
  if (flags.json)
    console.log(
      JSON.stringify({ command: 'test-audit', version: 1, status: 'error', error: message })
    )
